...
function setup() //setup function runs once at beginning
{
  ...
  //TYPICAL P5.JS SETUP
  createCanvas(1000, 720); //set size of canvas
...
function draw() //  draw function loops forever at frame rate
{
  ...
  push();
  translate(200, 400);
  rotate(frameCount * inData);
  rect(0, 0, 100, 200);
  pop();
  fourBit = Math.round(map(inData, 0, 255, 0, 15));
  fill(232, 255, 82);
  push();
  translate(450, 400);
  rotate(frameCount * fourBit);
  rect(0, 0, 100, 200);
  pop();
  twoBit = Math.round(map(inData, 0, 255, 0, 3));
  fill(81, 255, 69);
  push();
  translate(700, 400);
  rotate(frameCount * twoBit);
  rect(0, 0, 100, 200);
  pop();
  fill(0);
  textSize(30);
  text("Serial's pure 8-bit data: " + inData, 200, 200, 300, 300);
  text("Serial data mapped \nto 4 bits: " + fourBit, 475, 200, 600, 300);
  text("Serial data mapped \nto 2 bits: " + twoBit, 750, 200, 800, 300);
}
